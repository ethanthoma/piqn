...
from transformers.models.bert.modeling_bert import BertEmbeddings, BertOnlyMLMHead
...
class EntityEmbeddings(nn.Module):
    def __init__(self, config, is_pos_embedding=False):
        ...
        self.entity_embeddings = nn.Embedding(
            config.entity_queries_num, config.hidden_size
        )
        self.use_common_embedding = (
            config.use_entity_common_embedding and not is_pos_embedding
        )
    ...
    def forward(self, input_ids):
        ...
        if self.use_common_embedding:
            embeddings = embeddings + self.entity_common_embedding(
...
class EntityAwareBertSelfAttention(nn.Module):
    ...
    def forward(
        self, token_hidden_states, entity_hidden_states, attention_mask, query_pos=None
    ):
        ...
        if self.config.entity_aware_attention:
            e2w_query_layer = self.transpose_for_scores(
                self.entity_e2w_query(pos_aware_entity_hidden_states)
            )
        else:
            e2w_query_layer = self.transpose_for_scores(
                self.query(pos_aware_entity_hidden_states)
            )
        ...
        if self.config.entity_aware_attention:
            e2w_key_layer = self.transpose_for_scores(
                self.entity_e2w_key(pos_aware_entity_hidden_states)
            )
        else:
            e2w_key_layer = self.transpose_for_scores(
                self.key(pos_aware_entity_hidden_states)
            )

        w2w_value_layer = self.transpose_for_scores(self.value(token_hidden_states))

        if self.config.entity_aware_attention:
            e2w_value_layer = self.transpose_for_scores(
                self.entity_e2w_value(entity_hidden_states)
            )
        else:
            e2w_value_layer = self.transpose_for_scores(
                self.value(entity_hidden_states)
            )
        ...
        w2e_attention_scores = torch.matmul(
            w2w_query_layer, e2w_key_layer.transpose(-1, -2)
        )
        e2w_attention_scores = torch.matmul(
            e2w_query_layer, w2w_key_layer.transpose(-1, -2)
        )
...
class EntityAwareBertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        if config.entity_aware_selfout:
            ...
            self.entity_LayerNorm = nn.LayerNorm(
    ...
    def forward(
        ...
        if self.config.entity_aware_selfout:
            ...
            entity_self_output = self.entity_LayerNorm(
                entity_self_output + entity_hidden_states
            )
        else:
            entity_self_output = self.dense(entity_self_output)
            entity_self_output = self.dropout(entity_self_output)
            entity_self_output = self.LayerNorm(
                entity_self_output + entity_hidden_states
            )
...
class EntityAwareBertAttention(nn.Module):
    ...
    def forward(
        self, word_hidden_states, entity_hidden_states, attention_mask, query_pos=None
    ):
        ...
        return output[:, : word_hidden_states.size(1), :], output[
            :, word_hidden_states.size(1) :, :
        ]
...
class EntityAwareBertOutput(nn.Module):
    ...
    def forward(
        self,
        token_intermediate_output,
        entity_intermediate_output,
        ...
        token_intermediate_output = self.LayerNorm(
            token_intermediate_output + word_attention_output
        )
        ...
        if self.config.entity_aware_output:
            ...
            entity_intermediate_output = self.entity_LayerNorm(
                entity_intermediate_output + entity_attention_output
            )
        else:
            entity_intermediate_output = self.dense(entity_intermediate_output)
            entity_intermediate_output = self.dropout(entity_intermediate_output)
            entity_intermediate_output = self.LayerNorm(
                entity_intermediate_output + entity_attention_output
            )
...
class EntityAwareBertLayer(nn.Module):
    ...
    def forward(
        ...
        token_intermediate_output, entity_intermediate_output = self.intermediate(
            word_attention_output, entity_attention_output
        )
        token_output, entity_output = self.output(
            token_intermediate_output,
            entity_intermediate_output,
            word_attention_output,
            entity_attention_output,
...
class EntityAwareBertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [EntityAwareBertLayer(config) for _ in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        intermediate = [
            {"h_token": word_hidden_states, "h_entity": entity_hidden_states}
        ]
...
class SelfCrossAttention(nn.Module):
    def __init__(
        self, config, use_token_level_encoder, use_entity_attention, num_selfcrosslayer
    ):
        super().__init__()
        ...
        if self.use_entity_attention:
            ...
            self.selflaters = nn.ModuleList(
                [EntitySelfAttention(config) for _ in range(num_selfcrosslayer)]
            )

        self.crosslayers = None
        if self.use_token_level_encoder:
            self.crosslayers = nn.ModuleList(
                [EntityAwareBertLayer(config) for _ in range(num_selfcrosslayer)]
            )
    ...
    def forward(self, h_token, h_entity, token_entity_attention_mask, query_pos=None):
        ...
        for i in range(self.num_selfcrosslayer):
            if self.use_token_level_encoder:
                h_token, h_entity = self.crosslayers[i](
                    h_token, h_entity, token_entity_attention_mask, query_pos=query_pos
                )
...
class EntityBoundaryPredictor(nn.Module):
    ...
    def forward(self, token_embedding, entity_embedding, token_mask):
        ...
        entity_token_matrix = self.token_embedding_linear(token_embedding).unsqueeze(
            1
        ) + self.entity_embedding_linear(entity_embedding).unsqueeze(2)
        entity_token_cls = self.boundary_predictor(
            torch.relu(entity_token_matrix)
        ).squeeze(-1)
...
class EntityTypePredictor(nn.Module):
    ...
    def forward(self, h_entity, h_token, p_left, p_right, token_mask):
        ...
        attn_output, _ = self.multihead_attn(
            h_entity.transpose(0, 1).clone(),
            h_token.transpose(0, 1),
            h_token.transpose(0, 1),
...
class EntityAwareBertModel(BertPreTrainedModel):
    ...
    def _compute_extended_attention_mask(
        ...
        extended_attention_mask = (
            ...
            .unsqueeze(2)
            .expand(-1, -1, word_num + entity_num, -1)
            .clone()
        )

        # mask entity2token attention
        if (mask_ent2tok is None and self.config.mask_ent2tok) or mask_ent2tok is True:
            extended_attention_mask[:, :, :word_num, word_num:] = 0

        if (mask_tok2ent is None and self.config.mask_tok2ent) or mask_tok2ent is True:
            extended_attention_mask[:, :, word_num:, :word_num] = 0
        ...
        if (mask_ent2ent is None and self.config.mask_ent2ent) or mask_ent2ent is True:
            entity_attention = extended_attention_mask[:, :, word_num:, word_num:]
            mask = torch.eye(entity_num, entity_num, dtype=torch.bool).expand_as(
                entity_attention
            )
            entity_attention[~mask] = 0

        if (mask_entself is None and self.config.mask_entself) or mask_entself is True:
            entity_attention = extended_attention_mask[:, :, word_num:, word_num:]
            mask = torch.eye(entity_num, entity_num, dtype=torch.bool).expand_as(
                entity_attention
            )
    ...
    def forward(
        ...
        if entity_ids is None:
            entity_ids = torch.arange(
                self.config.entity_queries_num, device=token_input_ids.device
            ).expand((token_input_ids.size(0), -1))
            entity_attention_mask = torch.ones(
                entity_ids.size(), dtype=torch.long, device=token_input_ids.device
            )

        entity_embeddings = self.entity_embeddings(entity_ids)
        attention_mask = self._compute_extended_attention_mask(
            token_attention_mask, entity_attention_mask, seg_mask=None
        )
        ...
        return self.encoder(
            word_embeddings, entity_embeddings, attention_mask, query_pos=query_pos
        )
...
class PIQN(PreTrainedModel):
    ...
    def __init__(
        ...
        lstm_layers=3,
        pool_type: str = "max",
        word_mask_tok2ent=None,
        word_mask_ent2tok=None,
        word_mask_ent2ent=None,
        word_mask_entself=None,
        share_query_pos=False,
        use_token_level_encoder=True,
        num_token_entity_encoderlayer=1,
        ...
        self._keys_to_ignore_on_save = [
            "model." + k for k, v in self.model.named_parameters()
        ]
        ...
        self.entity_classifier = EntityTypePredictor(
            config, cls_size, entity_type_count
        )
        ...
        if self.use_token_level_encoder:
            self.selfcrossattention = SelfCrossAttention(
                config,
                use_token_level_encoder,
                use_entity_attention,
                self.num_token_entity_encoderlayer,
            )
        ...
        self.register_buffer(
            "entity_attention_mask", torch.ones(self.config.entity_queries_num)
        )
    ...
    def _common_forward(
        ...
        char_encoding: torch.tensor = None,
        token_masks_char=None,
        char_count: torch.tensor = None,
    ):
        ...
        h, h_entity, intermediate_subword_entity = self.model(
            token_input_ids=encodings,
            token_attention_mask=context_masks,
        ...
        for subword_entity_dic in intermediate_subword_entity:
            ...
            token = self.combine(
                subword_entity_dic["h_token"], context2token_masks, self.pool_type
            )
        ...
        def add_other_embedding(h_token, char_count, token_masks_char, char_encoding):
            ...
            if self.use_char_lstm:
                ...
                char_encoding = char_encoding.view(
                    max_token_count * batch_size, max_char_count
                )
                ...
                char_embed_packed = nn.utils.rnn.pack_padded_sequence(
                    input=char_embed,
                    lengths=char_count.tolist(),
                    enforce_sorted=False,
                    batch_first=True,
                )
                char_embed_packed_o, (_, _) = self.char_lstm(char_embed_packed)
                char_embed, _ = nn.utils.rnn.pad_packed_sequence(
                    char_embed_packed_o, batch_first=True
                )
                char_embed = char_embed.view(
                    batch_size, max_token_count, max_char_count, self.char_size * 2
                )
        ...
        h_token = add_other_embedding(
            h_token, char_count, token_masks_char, char_encoding
        )
        ...
        entity_attention_mask = torch.ones(
            h_entity.size()[:-1], dtype=torch.long, device=h_entity.device
        )
        ...
        token_entity_attention_mask = self.model._compute_extended_attention_mask(
            token_masks,
            entity_attention_mask,
            mask_ent2tok=self.word_mask_ent2tok,
            mask_tok2ent=self.word_mask_tok2ent,
            mask_ent2ent=self.word_mask_ent2ent,
            mask_entself=self.word_mask_entself,
        )

        query_pos = None
        if (
            self.config.use_entity_pos
            and self._share_query_pos
            and self.use_token_level_encoder
        ):
        ...
        intermediate = []
        if self.use_token_level_encoder:
            h_token, h_entity, intermediate = self.selfcrossattention(
                h_token, h_entity, token_entity_attention_mask, query_pos=query_pos
            )
        ...
        if self.inlcude_subword_aux_loss:
            for i, v in enumerate(intermediate_word_entity):
                ...
                if self.use_lstm:
                    ...
                    h_token, _ = nn.utils.rnn.pad_packed_sequence(
                        h_token, batch_first=True
                    )
        ...
        if self.use_aux_loss and len(intermediate) != 0:
            for h_dict in intermediate:
                ...
                entity_logits = self.entity_classifier(
                    h_entity, h_token, p_left, p_right, token_masks
                )
                output.append(
                    {
                        "p_left": p_left,
            ...
            output = [
                {"p_left": p_left, "p_right": p_right, "entity_logits": entity_logits}
            ]
    ...
    def _forward_train(
        ...
        if (
            ...
            and epoch >= self.split_epoch
            and not self.freeze_transformer
        ):
...
_MODELS = {"piqn": BertPIQN, "roberta_piqn": RobertaPIQN}
